...
angular.module('owsWalletApp.services').factory('coinbaseService', function($http, $log, $window, $filter, platformInfo, lodash, storageService, configService, appConfigService, txFormatService, buyAndSellService, $rootScope, feeService, networkService) {
  var root = {};
  var credentials = {};
  ...
  root.checkEnoughFundsForFee = function(amount, cb) {
    _getNetAmount(amount, function(err, reducedAmount) {
      ...
      // Check if transaction has enough funds to transfer bitcoin from Coinbase to this wallet
      if (reducedAmount < 0) {
