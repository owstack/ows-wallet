...
angular.module('owsWalletApp.services')
  .factory('trezorService', function($log, $timeout, lodash, hwWalletService, platformInfoService, networkService) {
    var root = {};
    ...
    root.description = {
      supported: platformInfoService.supportsTrezor,
      id: 'trezor',
      name: 'Trezor',
    ...
    root.signTx = function(xPubKeys, txp, account, callback) {
      ...
      if (txp.addressType == 'P2PKH') {
        ...
        inputs = lodash.map(txp.inputs, function(i) {
          ...
          var n = [hwWalletService.UNISIG_ROOTPATH | 0x80000000, 0 | 0x80000000, account | 0x80000000, parseInt(pathArr[1]), parseInt(pathArr[2])];
          inAmount += i.satoshis;
          return {
            address_n: n,
            prev_index: i.vout,
        ...
        inputs = lodash.map(txp.inputs, function(i) {
          $log.debug("Trezor TX input path:", i.path);
          var pathArr = i.path.split('/');
          var n = [hwWalletService.MULTISIG_ROOTPATH | 0x80000000, 0 | 0x80000000, account | 0x80000000, parseInt(pathArr[1]), parseInt(pathArr[2])];
          var np = n.slice(3);
      ...
      TrezorConnect.signTx(inputs, outputs, function(res) {
        if (!res.success)
          return callback(hwWalletService._err(res));
