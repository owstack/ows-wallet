...
angular.module('owsWalletApp.controllers').controller('ConfirmCtrl', function($rootScope, $scope, $interval, $timeout, $ionicScrollDelegate, gettextCatalog, walletService, platformInfoService, lodash, configService, $stateParams, $window, $state, $log, profileService, txFormatService, ongoingProcessService, $ionicModal, popupService, $ionicHistory, $ionicConfig, feeService, walletClientErrorService, txConfirmNotificationService, networkService) {
  ...
  function getSendMaxInfo(tx, wallet, cb) {
    ...
    //ongoingProcessService.set('retrievingInputs', true);
  ...
  $scope.openPPModal = function() {
    $ionicModal.fromTemplateUrl('views/modals/payment-protocol.html', {
      scope: $scope
    }).then(function(modal) {
  ...
  $scope.approve = function(tx, wallet, onSendStatusChange) {
    ...
    ongoingProcessService.set('creatingTx', true, onSendStatusChange);
    getTxp(lodash.clone(tx), wallet, false, function(err, txp) {
      ongoingProcessService.set('creatingTx', false, onSendStatusChange);
      if (err) return;
      ...
      function publishAndSign() {
        ...
        walletService.publishAndSign(wallet, txp, function(err, txp) {
          ...
          if (config.confirmedTxsNotifications && config.confirmedTxsNotifications.enabled) {
            txConfirmNotificationService.subscribe(wallet, {
              txid: txp.txid
            });
          }
        }, onSendStatusChange);
  ...
  $scope.chooseFeeLevel = function(tx, wallet) {
    ...
    $ionicModal.fromTemplateUrl('views/modals/choose-fee-level.html', {
      scope: scope,
