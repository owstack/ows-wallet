...
angular.module('owsWalletApp.controllers').controller('WalletHistorySettingsCtrl', function($rootScope, $scope, $log, $stateParams, $timeout, $ionicHistory, storageService, platformInfoService, profileService, lodash, appConfig, walletService, networkService, gettextCatalog, popupService) {
  ...
  $scope.csvHistory = function(cb) {
    ...
    getHistory(function(err, txs) {
      ...
      data.forEach(function(it, index) {
        ...
        _amount = (it.action == 'sent' ? '-' : '') + network.Unit(amount, 'atomic').toStandardUnit();
        _note = it.message || '';
        _comment = it.note ? it.note.body : '';

        if (it.action == 'moved') {
          _note += ' Moved:' + network.Unit(it.amount, 'atomic').toStandardUnit();
        }
        ...
        if (it.fees && (it.action == 'moved' || it.action == 'sent')) {
          var _fee = network.Unit(it.fees, 'atomic').toStandardUnit();
