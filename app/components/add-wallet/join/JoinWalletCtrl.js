...
angular.module('owsWalletApp.controllers').controller('JoinWalletCtrl', function($scope, $rootScope, $timeout, $state, $ionicHistory, $ionicScrollDelegate, profileService, configService, gettextCatalog, lodash, ledgerService, trezorService, derivationPathService, ongoingProcessService, walletService, $log, $stateParams, popupService) {
  ...
  $scope.onSecretChange = function() {
    ...
    profileService.getNetworkFromJoinSecret($scope.formData.secret, function(err, network) {
      ...
      $scope.formData.walletServiceUrl = config.networkPreferences[network.name].walletService.url;
  ...
  $scope.join = function() {
    ...
    profileService.getNetworkFromJoinSecret($scope.formData.secret, function(err, network) {
      ...
      if (setSeed) {
        ...
        if (!pathData || (pathData.networkName != opts.network.name)) {
