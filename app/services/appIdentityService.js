...
angular.module('owsWalletApp.services').factory('appIdentityService', function($log, lodash, storageService, bitauthService) {
  var root = {};
  ...
  root.getIdentity = function(network, cb) {
    ...
    storageService.getAppIdentity(network, function(err, data) {
      ...
      try {
        ...
        identityNumber = bitauthService.getSinFromPublicKey(pubkey);
        if (isNew) {
